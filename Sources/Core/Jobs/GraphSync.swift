...
extension GraphSyncJob {
	...
	public static func sync(env: SyncEnvironment, filters: SyncFilters) async throws -> SyncSummary {
		...
		env.logger.info("Graph sync initiated")
		...
		let workers = try await Worker.dbUpsertAll(from: users, on: env.db)
		...
		env.logger.debug("Graph sync start for \(teams.count) teams")
		...
		for team in teams {
			do {
				...
				env.logger.info("Sync team \(team.id) (\(team.displayName ?? ""))")
				...
				env.logger.debug("Found \(listTimeOffReasons.count) time off reasons")
				let listTimeOffRequests = try await env.graphClient.listTimeOffRequests(teamId: team.id, from: from, to: to, client: env.client)
