...
extension TimeEntry {
	...
	public static func upsert(
		...
		for card in cards {
			...
			var worker: Worker
			if let w = workersByGraphId[userId] {
				worker = w
			} else {
				db.logger.debug("No worker match for userId=\(userId); creating placeholder worker")
				db.logger.critical("No worker match for userId=\(userId); creating placeholder worker")
				continue
			}
